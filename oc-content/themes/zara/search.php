...
<body <?php 
if(isset($_GET['page']) && isset($_GET['sCategory']) ){
  $page_type = $_GET['page'];
  $sCategory = $_GET['sCategory'];
   if($page_type == 'search'){
     if($sCategory != '')
      echo 'id="brend_'. $sCategory.'"';
  }
}
?> >
  ...
  <div id="sidebar" class="noselect">
                ...
                <select id="regionId" name="sRegion" <?php if(Params::getParam('sRegion') == '' && Params::getParam('region')) {?>disabled<?php } ?>>
                  ...
                  <?php if(isset($aRegions) && !empty($aRegions) && $aRegions <> '' && count($aRegions) >= 1) { ?>
                    <?php foreach ($aRegions as $region) {?>
                      <option value="<?php echo $region['pk_i_id']; ?>" <?php if(Params::getParam('sRegion') == $region['pk_i_id'] or Params::getParam('sRegion') == $region['s_name']) { ?>selected="selected"<?php } ?>><?php _e( $region['s_name'], 'zara'); ?></option>
                    <?php } ?>
                  <?php } ?>
                </select>
              <?php } else { ?>
    ...
    <?php if(osc_get_preference('theme_adsense', 'zara_theme') == 1) { ?>
      <div class="search-google">
        <?php echo osc_get_preference('banner_search', 'zara_theme'); ?>
      </div>        
    <?php } ?>
  </div>
</div>
