...
<div class="content user_account fi_user_menu_wrap">
  ...
  <div id="fi_user_new_list">
    <form name="fi_new_list" action="<?php echo osc_route_url('favorite-lists', array('list-id' => '0', 'current-update' => '0', 'notification-update' => '0', 'list-remove' => '0', 'iPage' => '0')); ?>" method="POST" enctype="multipart/form-data">
      ...
      <input type="text" class="fi_new_name" name="fi_new_list_name" placeholder="<?php _e('Enter name of new favorite list', 'favorite_items'); ?>"/>
  ...
  <div id="fi_user_lists">
    ...
    <?php } else { ?> 
      ...
      <?php foreach($lists as $l) { ?>
        ...
        <div class="fi_list fi_list_<?php echo $l['list_id']; ?>" rel="<?php echo $l['list_id']; ?>">
          ...
          <div class="fi_count"><?php echo $count; ?> <?php echo $count == 1 ? __('listing', 'favorite_items') : __(' ', 'favorite_items'); ?></div>
    ...
    <div class="fi_explain">
      <div><sup style="width: 8px;">*</sup><?php _e('Every user can have just 1 favorite list active/current. It only means, that if you add item to favorite list, it is added to active one. You can set favorite list to be active if you are going to work with it (add/remove items from it).', 'favorite_items'); ?></div>
