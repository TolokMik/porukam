...
function fi_config_menu() {
  ...
  $text .= '<li class="reenable"><a href="' . osc_base_url() . 'oc-admin/index.php?page=plugins&action=renderplugin&file=favorite_items/admin/configure.php&reenable=1" title="' . __('This action set all settings to default. In case you have upgraded from lower version, it will add also missing variables so reinstall is not required', 'favorite_items') . '" onclick="return confirm(\'' . __('This action set all settings to default. In case you have upgraded from lower version, it will add also missing variables so reinstall is not required. Are you sure you want to continue?', 'favorite_items') . '\')"><i class="fa fa-refresh"></i></a></li>';
  $text .= '</ul>';
  $text .= '</fieldset>';
  $text .= '<br /><br />';
