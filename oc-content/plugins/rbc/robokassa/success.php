   ...
   // echo 'type: '.$type. ' shpBype: '.$shpBype;
    ...
    if ($type == 1) {
        if (!($mp->premiumFeeIsPaid_rbc($shpBype))) {
		 osc_add_flash_ok_message(__('Payment processed correctly. Your ad will be published in the next issue of newspaper «Stolitsa»', 'rbc'));
		 $item = Item::newInstance()->findByPrimaryKey($shpBype);
		 $category = Category::newInstance()->findByPrimaryKey($item['fk_i_category_id']);
         View::newInstance()->_exportVariableToView('category', $category);
         if($item['fk_i_user_id'] <> 0) {
            $item_user = User::newInstance()->findByPrimaryKey($item['fk_i_user_id']);
         }
         $mobile = '';
         if($mobile == '') { $mobile = $item['s_city_area']; }      
         if($mobile == '' && $item['fk_i_user_id'] <> 0) { $mobile = $item_user['s_phone_mobile']; }      
         if($mobile == '' && $item['fk_i_user_id'] <> 0) { $mobile = $item_user['s_phone_land']; }      
        ...
        $howmoney = (float) $item['i_price']/1000000;
        $listing_url = '<p><br/><a href="http://porukam.kiev.ua/index.php?page=item&id='.$shpBype .'">Посмотреть объявление </a></p>';
        mail('admin<main@design-plus.kiev.ua>, elena@med-expert.com.ua',"Оплата ExPRESS - Portmone", 
        'Текст сообщения:<br/> '.$item['s_description'].'<br/> Стоимость:'.$howmoney.'<br/>тел.: '.$mobile. $listing_url, $options);
